...
(function () {
    ...
    this.setEntity = function (objeto) {
        ...
        entity.IdRol = objeto.IdRol;
        entity.EsActivo = objeto.EsActivo;
        entity.FechaContratacion = objeto.FechaContratacion;
...
var getFormValues = function () {
    return {
        ...
        IdPersona: $('#selIdPersona').val(),
        IdRol: $('#selIdRol').val(),
        EsActivo: $('#chkEsActivo').is(':checked') ? 1 : 0
    };
}
...
var setFormValuesEdit = function () {
    ...
    $('#selIdPersona').val(e.IdPersona);
    $('#selIdRol').val(e.IdRol);
    $('#chkEsActivo').prop('checked', e.EsActivo == 1);
}
...
var initData = function () {
    callAjax(null, urlGetAllRoles, "GET").done(function (data) {
        ClaseGlobalVar.roles = data;
        initDataTable();
        initEvent();
        loadPersonas();
        loadRoles();
...
var loadPersonas = function () {
    $.get('/api/admin/personas/available', function (data) {
        var sel = $('#selIdPersona');
        sel.find('option:not(:first)').remove();
        data.forEach(function (p) {
            sel.append($('<option>', {
                value: p.Id,
                text: p.Nombres + ' ' + p.Apellidos + ' (' + p.DNI + ')'
            }));
        });
    });
};
...
var loadRoles = function () {
    $.get('/api/admin/roles/all', function (data) {
        var sel = $('#selIdRol');
        sel.find('option:not(:first)').remove();
        ...
        callAjax(ClaseRegistro.pagedItem, urlGetPaged, "POST").done(function (data) {
            ...
            data.forEach(function (item, index) {
                ...
                var row = $("<tr>").append(
                    ...
                    $("<td>", { class: "text-center" }).text(ClaseGlobalVar.roles.find(r => r.Id == item.IdRol).Nombre),
                    $("<td>", { class: "text-center" }).text(item.EsActivo == 1 ? "Sí" : "No"),
